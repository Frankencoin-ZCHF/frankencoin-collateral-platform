...
const riskParameters: SystemBlock<RiskParametersData> = {
  ...
  load: async (ctx) => {
    const rows = compareParameters(ctx.assessment!.data.params, ctx.record.live);
    return { rows, hasLive: (ctx.record.live?.positions.active ?? 0) > 0, differs: rows.filter((r) => r.verdict === "differs").length };
  },
  Component: Block,
};
